...
int selectPolygon(cv::Mat srcMat, cv::Mat& dstMat1, cv::Mat& dstMat2)
{
	...
	cv::Mat n = cv::Mat::zeros(srcMat.size(), CV_32F);
	...
	n = 0;
	drawContours(n, contours, 0, Scalar(1), -1);
...
int calcVisbalDft(cv::Mat src, cv::Mat & magMat, cv::Mat & ph, double & normVal)
{
...
int calcDft2Image(cv::Mat magMat, cv::Mat ph, double normVal, cv::Mat & dstMat)
{
	...
	//planes[0]为dft变换的实部，planes[1]为虚部，ph为相位， plane_true=mag为幅值
	Mat planes[] = { Mat_<float>(mag.size()), Mat::zeros(mag.size(), CV_32F) };
...
int mouseROI()
{
	...
	selectPolygon(srcMat, dstMat1, dstMat2);
	imshow("select Area1", dstMat1);
	imshow("select Area2", dstMat2);
